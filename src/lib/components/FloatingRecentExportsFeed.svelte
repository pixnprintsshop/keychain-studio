...
{#if showWidget}
	...
	>
		{#if recentExportFeed.toastEvent && !recentExportFeed.expanded}
			...
			>
				<span
					class="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-700"
					aria-hidden="true"
				>
					<svg
		...
		<div class="pointer-events-auto w-full">
			...
			{#if recentExportFeed.expanded}
				<div
					class="mt-2 max-h-56 overflow-y-auto rounded-xl border border-slate-200/90 bg-white/95 p-1.5 shadow-md backdrop-blur-sm"
				>
					<ul class="space-y-1">

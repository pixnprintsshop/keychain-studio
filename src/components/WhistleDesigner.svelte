<script lang="ts">
    ...
    import whistleBaseStlUrl from "../assets/stl/whistle-base.stl?url";
        ...
        if (content) {
            ...
            if (font) {
                ...
                if (shapes.length > 0) {
                    ...
                    const scaleCap = (whistleW * 0.8) / (tb.max.x - tb.min.x || 0.01);
                    const scaleCapped = Math.min(scale, scaleCap);
                    textMesh.scale.set(scaleCapped, scaleCapped, 1);
                    textMesh.position.set(centerX + textOffsetX, centerY, topZ + 0.02);
                    group.add(textMesh);
                }
            }
        }
    ...
    <div class="mx-auto flex h-full w-full max-w-[1600px] flex-col gap-4 lg:flex-row min-h-0">
            ...
            <div class="min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden pr-1 p-4 pt-0">
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label for="whistle-text-scale" class="text-xs font-medium text-slate-700">
                            Text scale
                        </label>
                        <span class="text-xs text-slate-500">{textScale.toFixed(1)}×</span>
                    </div>
                    <input
                        id="whistle-text-scale"
                        type="range"
                        min="0.3"
                        max="2"
                        step="0.1"
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label for="whistle-text-depth" class="text-xs font-medium text-slate-700">
                            Text depth
                        </label>
                        <span class="text-xs text-slate-500">{textDepth.toFixed(1)} mm</span>
                    </div>
                    <input
                        id="whistle-text-depth"
                        type="range"
                        min="0.2"
                        max="2"
                        step="0.1"
                ...
                <div>
                    <label class="flex items-center gap-2" for="whistle-text-color">
                        <span class="text-xs font-medium text-slate-700">Text color</span>
                        <input
                            id="whistle-text-color"
                ...
                <div>
                    <label class="flex items-center gap-2" for="whistle-base-color">
                        <span class="text-xs font-medium text-slate-700">Base color</span>
                        <input
                            id="whistle-base-color"
                            class="h-10 w-10 rounded-xl cursor-pointer"
                            type="color"
        ...
        >
            ...
            <div class="absolute bottom-4 right-4 flex items-center gap-2">
                ...
                <button
                    class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold tracking-tight text-slate-900 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                    type="button"
                    onclick={() => void exportStl()}
                    disabled={!whistleGeometry ||
                        exportLoading ||
                        !user ||

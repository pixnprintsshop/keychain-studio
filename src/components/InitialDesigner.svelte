<script lang="ts">
  ...
  function rebuildMeshes() {
      ...
      const buildFromOuter = (outerNode: any) => {
        ...
        for (const ch of children) {
          ...
          if (holeContour.length >= 3)
            shape.holes.push(new THREE.Path(holeContour.map(toVec2)));
    ...
    const initialCharStr = (text?.charAt(0)?.toUpperCase() as string) || "A";
    ...
    if (keyringEnabled) {
      const ringGeo = makeKeyringGeometry(
        ...
        Math.max(0.1, Math.min(keyringHoleSize, keyringOuterSize - 0.5)),
    ...
    >
      ...
      <div
        ...
        <div class="grid grid-cols-1 gap-4">
          ...
          <!-- Text Font -->
          <label class="grid gap-1.5">
            <span class="text-xs font-medium text-slate-700">Text Font</span>
                ...
                >
                  {#each FONT_OPTIONS as f, index}
                    ...
                    {@const isTrialRestricted =
                      user && licenseStatus?.type === "trial" && !isFirstFont}
                    <button
                      type="button"
          ...
          <!-- Initial Font -->
          <label class="grid gap-1.5">
            <span class="text-xs font-medium text-slate-700">Initial Font</span>
            <div class="relative" bind:this={initialFontDropdownRef}>
              <button
                type="button"
                class="w-full appearance-none rounded-xl border border-slate-200 bg-white px-3 py-2 text-left text-sm text-slate-900 shadow-sm outline-none ring-indigo-500/25 focus:border-indigo-400 focus:ring-2 flex items-center justify-between"
                onclick={(e) => {
                  e.stopPropagation();
                  initialFontDropdownOpen = !initialFontDropdownOpen;
                }}
              >
                <span
                  style="font-family: {FONT_OPTIONS.find(
                ...
                >
                  {FONT_OPTIONS.find((f) => f.key === initialFontKey)?.label ||
                    "Select font"}
                </span>
                <svg
          ...
          <label class="grid gap-1.5">
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="text-xs tabular-nums text-slate-600">{textSize}</span
              >
            </div>
            <input
              class="w-full accent-indigo-500"
    ...
    >
      ...
      <div class="absolute bottom-4 right-4">
        <DesignerExportToolbar
          onSnapshot={() =>
            downloadSnapshot(renderer, scene, camera, "keychain-initial")}
          onExport={exportSTL}

<script lang="ts">
        ...
        if (content) {
            ...
            if (font) {
                ...
                if (shapes.length > 0) {
                    ...
                    const scaleCap =
                        (baseW * 0.8) / (tb.max.x - tb.min.x || 0.01);
    ...
    async function openWithBambuStudio() {
        ...
        try {
            ...
            const blob = await exportTo3MF(exportGroup);
            if (!blob || blob.size === 0) return;
            const publicUrl = await upload3mfToSupabase(blob, 'stanley-topper');
            notifyExportEvent({
                email: user?.email,
                name: (user?.user_metadata?.full_name as string) ?? (user?.user_metadata?.name as string),
                subscriptionStatus,
        ...
        <aside
            ...
            <div
                ...
                <p class="text-xs text-slate-500">
                    Add your text on top of the Stanley topper. Text is centered
                    and scaled to fit.
                </p>
                ...
                <div>
                    ...
                    <FontSelect id="stanley-font" bind:value={fontKey} />
                </div>

                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            for="stanley-text-scale"

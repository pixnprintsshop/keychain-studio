<script lang="ts">
    ...
    import stanleyTopperStlUrl from "../assets/stl/Stanley Topper.stl?url";
    import type { LicenseStatus } from "../lib/licensing";
    import {
      centerGeometryXY,
      disposeObject3D,
      downloadBlob,
      downloadSnapshot, FONT_OPTIONS, frameCameraToObject, getFont
    } from "../lib/utils";
    ...
    const defaultSettings = {
        textContent: "Name",
        fontKey: "Beautiful Harmony_Regular",
        textSize: 36,
        textScale: 1,
        textDepth: 1,
        textColor: "#ffffff",
        ...
        if (content) {
            ...
            if (font) {
                ...
                if (shapes.length > 0) {
                    ...
                    // Target height scales with textSize so the slider actually changes visible size
                    const TARGET_HEIGHT_MM = 10 * (textSize / 20) * textScale;
                    const scale = TARGET_HEIGHT_MM / textH;
                    const scaleCap = (baseW * 0.8) / (tb.max.x - tb.min.x || 0.01);
                    const scaleCapped = Math.min(scale, scaleCap);
                    textMesh.scale.set(scaleCapped, scaleCapped, 1);
                    textMesh.position.set(centerX, centerY, topZ + 0.02);
                    group.add(textMesh);
                }
            }
        }
    ...
    onMount(() => {
        ...
        loader.load(stanleyTopperStlUrl, (geometry: THREE.BufferGeometry) => {
            centerBase(geometry);
        ...
        >
            ...
            <div class="min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden p-4 pt-0">
                <p class="text-xs text-slate-500">
                    Add your text on top of the Stanley topper. Text is centered and scaled to fit.
                </p>

                <div>
                    <label
                        for="stanley-text-input"
                        class="mb-1 block text-xs font-medium text-slate-700"
                    >
                        Text
                    </label>
                    <input
                        id="stanley-text-input"
                        type="text"
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label for="stanley-text-scale" class="text-xs font-medium text-slate-700">
                            Text scale
                        </label>
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label for="stanley-text-depth" class="text-xs font-medium text-slate-700">
                            Text depth
                        </label>
                ...
                <div>
                    <label class="flex items-center gap-2" for="stanley-text-color">
                        <span class="text-xs font-medium text-slate-700">Text color</span>
                        <input
                ...
                <div>
                    <label class="flex items-center gap-2" for="stanley-base-color">
                ...
                <DesignerExportToolbar
                    onSnapshot={() => {
                        if (renderer && scene && camera)
                            downloadSnapshot(renderer, scene, camera, "stanley-topper");
                    }}
